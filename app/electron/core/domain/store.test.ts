...
import { countDescendants } from "../sync/delete-cost.js";
import { adoptStray, scanStrays } from "../workspace/doctor.js";
import { buildTree, rebuildIndex } from "../workspace/rebuild-index.js";
...
test("updateIssue CAS rejects stale expected baseline for description", async () => {
  await withWorkspace(async (root, projectId) => {
    ...
    const { pickIssueEditable, StaleWriteError } = await import("../sync/detail-diff.js");
...
test("updateIssue CAS allows write when expected matches disk", async () => {
  await withWorkspace(async (root, projectId) => {
    ...
    const { pickIssueEditable } = await import("../sync/detail-diff.js");
    const expected = pickIssueEditable(issue);
