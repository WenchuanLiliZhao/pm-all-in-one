...
electron/workspace-template/   # → new workspace root
  ...
  .agents/skills/
    ...
    custom/.gitkeep              # required empty dir; git cannot track a bare folder
  …
...
- The graph walk is what keeps Electron-importing siblings (`electron/core/workspace/settings.ts`) out of the package. Dependency versions are read from app `package.json`; the build fails rather than guessing if the CLI grows an import that is not declared there.
...
| Dep | Used by |
| --- | --- |
| `zod` | `electron/core/domain/custom-props.ts`, `infra/props-load.ts`, `infra/timestamps.ts`, `views/views.ts`, `views/view-orders.ts`, `domain/wiki.ts` |
| `esbuild` | `electron/core/infra/props-load.ts`, `domain/wiki.ts` (evaluates `props.ts`) |
| `nanoid` | `electron/core/identity/ids.ts`, `views/views.ts` |
| `chokidar` | `electron/core/workspace/watch.ts` |
| `node-pty` | `electron/core/desktop/pty.ts` (native; stays unpacked) |
...
- Hierarchy lives in `props.ts`: `parentId` is the only authority for the tree, and `level` (epic → task → subtask) is stated alongside it. When the two disagree, both are kept and a violation is reported — see `electron/core/identity/ladder.ts` and dogfood `@wiki-WZ_eBxLpaAG_HYKecNZeW`.
...
| _(no `src/lib` twin)_ | `electron/core/sync/detail-diff.ts` via `@pm-core/sync/detail-diff` | Shared core / contract-adjacent; not a third bridge |
