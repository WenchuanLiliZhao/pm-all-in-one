...
- **Browser** — no preload; `getPm()` → HTTP client; Vite proxies `/api` → `8787`. Fixed workspace via `LOCAL_PM_WORKSPACE` (default `/Users/wenchuanzhao/Desktop/PMs/new-world`). No folder picker / no real terminal yet.
...
| Env | Default | Meaning |
| --- | --- | --- |
| `LOCAL_PM_WORKSPACE` | `/Users/wenchuanzhao/Desktop/PMs/new-world` | Workspace path (relative to `space/app` cwd, or absolute) |
...
Dogfood library: `/Users/wenchuanzhao/Desktop/PMs/new-world`. Nodes / wiki disk pattern: [[../important-notes/nodes|nodes]].
...
- Hierarchy lives in `props.ts`: `parentId` is the only authority for the tree, and `level` (epic → task → subtask) is stated alongside it. When the two disagree, both are kept and a violation is reported — see `core/ladder.ts` and [[../important-notes/nodes|nodes]].
...
| Zone | Owns |
| --- | --- |
| 1 Spec | Repo `AGENTS.md`, `space/important-notes/`, `space/handoff/` |
