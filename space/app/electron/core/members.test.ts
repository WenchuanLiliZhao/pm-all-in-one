...
import { parseMemberLinks, memberLinkSyntax } from "./links.js";
...
test("updateMember CAS rejects stale expected baseline for body", async () => {
  await withTempWorkspace(async (root) => {
    ...
    const { pickMemberEditable, StaleWriteError } = await import("./detail-diff.js");
...
test("updateMember CAS allows write when expected matches disk", async () => {
  await withTempWorkspace(async (root) => {
    ...
    const { pickMemberEditable } = await import("./detail-diff.js");
    const expected = pickMemberEditable(m);
