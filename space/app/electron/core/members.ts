 ...
 *
 * ↔ electron/core/detail-diff.ts — OCC expected / StaleWriteError on updateMember
...
import { parseId, type EntityId } from "./dir-id.js";
import {
  equalsForSync,
  pickMemberEditable,
  StaleWriteError,
...
import { allocateMemberId, membersRoot } from "./ids.js";
import { loadMemberProps, writePropsTs } from "./props-load.js";
import {
  isIsoDateTimeZ,
  nowIsoUtcZ,
  resolveTimestamps,
...
export async function backfillMemberRefs(
  ...
  for (const issue of await listIssues(workspaceRoot)) {
    ...
    const { loadIssueProps } = await import("./props-load.js");
    const { LEVEL_TYPE_NAME } = await import("./schema-dts.js");
    const props = {
...
async function loadMemberPropsForBackfill(
  ...
  const { evaluatePropsExport } = await import("./props-load.js");
